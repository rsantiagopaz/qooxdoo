qx.Class.define("inventario.comp.paramet.windowTipo_bien",
{
	...
	commandAgregar.addListener("execute", function(e){
		...
		tableModel.addRowsAsMapArray([{id_tipo_bien: "0", descrip: "Nuevo tipo bien", alta: true, modificado: false, eliminado: false}], null, true);
	...
	menu.memorizar();
		...
		btnAceptar.addListener("execute", function(e){
			...
			if (enviar) {
				...
				try {
					var resultado = rpc.callSync("escribir_tipo_bien", p);
				} catch (ex) {
		...
		var rpc = new qx.io.remote.Rpc("services/", "comp.Paramet");
		try {
			var resultado = rpc.callSync("leer_tipo_bien");
		} catch (ex) {
			alert("Sync exception: " + ex);
		}
