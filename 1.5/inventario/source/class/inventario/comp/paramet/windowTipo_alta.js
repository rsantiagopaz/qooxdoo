qx.Class.define("inventario.comp.paramet.windowTipo_alta",
{
	...
	commandAgregar.addListener("execute", function(e){
		...
		tableModel.addRowsAsMapArray([{id_tipo_alta: "0", descrip: "Nuevo tipo alta", alta: true, modificado: false, eliminado: false}], null, true);
	...
	menu.memorizar();
		...
		btnAceptar.addListener("execute", function(e){
			...
			if (enviar) {
				...
				try {
					var resultado = rpc.callSync("escribir_tipo_alta", p);
				} catch (ex) {
		...
		var rpc = new qx.io.remote.Rpc("services/", "comp.Paramet");
		try {
			var resultado = rpc.callSync("leer_tipo_alta");
		} catch (ex) {
			alert("Sync exception: " + ex);
		}
