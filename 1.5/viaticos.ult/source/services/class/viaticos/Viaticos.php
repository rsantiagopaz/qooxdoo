...
class class_Viaticos extends class_Base
{
  ...
  public function method_validar_alta_modifica_viatico($params, $error) {
	...
	$formato = "Y-m-d H:i:s";
	$fecha_desde = date_create(substr($p->model->fecha_desde2, 0, 10) . " " . $p->model->hora_desde2);
	$fecha_hasta = date_create(substr($p->model->fecha_hasta2, 0, 10) . " " . $p->model->hora_hasta2);
  	
	if (!is_null($p->model->COD_VEHICULO)) {
	  	$sql = "SELECT id_viatico FROM viatico WHERE estado <> 'A' AND id_viatico <> '" . $p->model->id_viatico . "' AND documentacion_id <> '" . $p->model->documentacion_id . "' AND COD_VEHICULO='" . $p->model->COD_VEHICULO . "' AND GREATEST(ADDTIME(fecha_desde2, hora_desde2), '" . $fecha_desde->format($formato) . "') - LEAST(ADDTIME(fecha_hasta2, hora_hasta2), '" . $fecha_hasta->format($formato) . "') < 0";
		$rs = mysql_query($sql);
  	...
  	$sql = "SELECT id_viatico FROM viatico WHERE estado <> 'A' AND id_viatico <> '" . $p->model->id_viatico . "' AND id_personal='" . $p->model->id_personal . "' AND GREATEST(ADDTIME(fecha_desde2, hora_desde2), '" . $fecha_desde->format($formato) . "') - LEAST(ADDTIME(fecha_hasta2, hora_hasta2), '" . $fecha_hasta->format($formato) . "') < 0";
	$rs = mysql_query($sql);
	if (mysql_num_rows($rs) > 0) {
		$item = new stdClass();
	...
	if ($p->validar_tope) {
		$json = json_decode($p->model->json);
		
		$fd = date_create(substr($p->model->fecha_desde2, 0, 10) . " " . $p->model->hora_desde2);
		
		$fecha_desde_info = getdate($fecha_desde->getTimestamp());
		$fecha_hasta_info = getdate($fecha_hasta->getTimestamp());

		do {
			$fd_info = getdate($fd->getTimestamp());
			$fh = date_create($fd_info["year"] . "-" . $fd_info["mon"] . "-" . "01");
			$fh->add(new DateInterval("P1M"));
			if ($fh > $fecha_hasta) $fh = $fecha_hasta;
			$diff = $fd->diff($fh);
			
			if ($p->model->fuera_provincia) {
				$horas = ((int) $diff->format("%d") * 24) + (int) $diff->format("%h");
	...
	if ($p->validar_tope) {
	  	$sql = "SELECT SUM(subtotal_viatico2) AS total, id_personal FROM viatico WHERE estado <> 'A' AND id_viatico <> '" . $p->model->id_viatico . "' AND id_personal='" . $p->model->id_personal . "' AND YEAR(fecha_desde2)=" . substr($fecha_desde->format($formato), 0, 4) . " AND MONTH(fecha_desde2)=" . substr($fecha_desde->format($formato), 5, 2) . " GROUP BY id_personal";
		$rs = mysql_query($sql);
		if (mysql_num_rows($rs) > 0) {
  ...
  public function method_autocompletarOrganismoArea($params, $error) {
  	$p = $params[0];
	$sql = "SELECT CONCAT(organismo_area, ' (', organismo, ')') AS label, organismo_area_id AS model FROM salud1._organismos_areas INNER JOIN salud1._organismos USING(organismo_id) WHERE organismo_area LIKE '%" . $p->texto . "%' ORDER BY label";
	return $this->toJson($sql);
  }
