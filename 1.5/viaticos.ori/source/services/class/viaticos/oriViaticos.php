  ...
  public function method_validar_alta_modifica_viatico($params, $error) {
	...
	if ($p->validar_tope) {
		...
		$bool2 = false;
	  	...
	  	$sql = "SELECT * FROM viatico WHERE estado <> 'A' AND id_viatico <> '" . $p->model->id_viatico . "' AND id_personal='" . $p->model->id_personal . "' AND YEAR(fecha_desde2)=" . substr($fecha_desde, 0, 4) . " AND MONTH(fecha_desde2)=" . substr($fecha_desde, 5, 2);
		$rs = mysql_query($sql);
		if (mysql_num_rows($rs) > 0) {
			while ($row = mysql_fetch_object($rs)) {
				if (substr($row->fecha_desde2, 0, 4) == substr($row->fecha_hasta2, 0, 4) && substr($row->fecha_desde2, 5, 2) == substr($row->fecha_hasta2, 5, 2)) {
					$total+= (float) $row->subtotal_viatico2;
				} else {
		...
		if ($total + $p->model->subtotal_viatico2 > $p->model->codigo_002per * $porc_tope_cargo / 100) $bool1 = true;
		
		
		
	  	$sql = "SELECT SUM(subtotal_viatico2) AS total, id_personal FROM viatico WHERE estado <> 'A' AND id_viatico <> '" . $p->model->id_viatico . "' AND id_personal='" . $p->model->id_personal . "' AND YEAR(fecha_hasta2)=" . substr($fecha_hasta, 0, 4) . " AND MONTH(fecha_hasta2)=" . substr($fecha_hasta, 5, 2) . " GROUP BY id_personal";
		$rs = mysql_query($sql);
		if (mysql_num_rows($rs) > 0) {
			$row = mysql_fetch_object($rs);
		...
		if ($total + $p->model->subtotal_viatico2 > $p->model->codigo_002per * $porc_tope_cargo / 100) $bool2 = true;
	...
	if ($p->model->estado != "L" && $p->model->estado != "R") {
