qx.Class.define("viaticos.viaticos.windowViatMen",
{
	...
	var sumar = function() {
		...
		if (!lstPersonal.isSelectionEmpty() && lstPersonal.getSelection()[0].getUserData("datos").funcionario) {
			json.diario.diario_dp = json.diario.diario_fp = txtCodigo_002per.getValue() * 4.5 / 100;
			json.diario.diario_12_dp = json.diario.diario_12_fp =  json.diario.diario_dp * 50 / 100;
			json.diario.diario_34_dp = json.diario.diario_34_fp = json.diario.diario_dp * 75 / 100;
		} else {
			json.diario.diario_12_dp = json.paramet.diario_12_dp;
			json.diario.diario_34_dp = json.paramet.diario_34_dp;
			json.diario.diario_dp = json.paramet.diario_dp;
			json.diario.diario_12_fp = json.paramet.diario_12_fp;
			json.diario.diario_34_fp = json.paramet.diario_34_fp;
			json.diario.diario_fp = json.paramet.diario_fp;
		}
		
		lbldiario_12_dp.setValue("* " + numberformatMontoEs.format(json.diario.diario_12_dp));
		lbldiario_34_dp.setValue("* " + numberformatMontoEs.format(json.diario.diario_34_dp));
		lbldiario_dp.setValue("* " + numberformatMontoEs.format(json.diario.diario_dp));

		lbldiario_12_fp.setValue("* " + numberformatMontoEs.format(json.diario.diario_12_fp));
		lbldiario_34_fp.setValue("* " + numberformatMontoEs.format(json.diario.diario_34_fp));
		lbldiario_fp.setValue("* " + numberformatMontoEs.format(json.diario.diario_fp));
		
		aux = txtDp12.getValue() * json.diario.diario_12_dp;
		subtotal_viatico2+= aux;
		lblSubtotD12.setValue(numberformatMontoEs.format(aux));
		
		aux = txtDp34.getValue() * json.diario.diario_34_dp;
		subtotal_viatico2+= aux;
		lblSubtotD34.setValue(numberformatMontoEs.format(aux));
		
		aux = txtDp1.getValue() * json.diario.diario_dp;
		subtotal_viatico2+= aux;
		lblSubtotD1.setValue(numberformatMontoEs.format(aux));
		
		aux = txtFp12.getValue() * json.diario.diario_12_fp;
		subtotal_viatico2+= aux;
		lblSubtotF12.setValue(numberformatMontoEs.format(aux));
		
		aux = txtFp34.getValue() * json.diario.diario_34_fp;
		subtotal_viatico2+= aux;
		lblSubtotF34.setValue(numberformatMontoEs.format(aux));
		
		aux = txtFp1.getValue() * json.diario.diario_fp;
		subtotal_viatico2+= aux;
	...
	/*
	var cboOrganismoArea = new controles.combobox.ComboBoxAuto("services/", "viaticos.Viaticos", "autocompletarOrganismoArea");
	...
	*/
	...
	var cboOrganismoAreaOrigen = new controles.combobox.ComboBoxAuto("services/", "viaticos.Viaticos", "autocompletarOrganismoArea");
	...
	var cboPersonal = new controles.combobox.ComboBoxAuto("services/", "viaticos.Viaticos", "autocompletarPersonal");
	...
	txtCodigo_002per.addListener("changeValue", sumar);
	...
	var cboMotivo = new controles.combobox.ComboBoxAuto("services/", "viaticos.Viaticos", "autocompletarMotivo");
	...
	form.add(slbCtaCte, "Cta.Cte.", null, "id_cta_cte", null, {enabled: estado=="L", grupo: 1, item: {row: 13, column: 16, colSpan: 10}});
	
	
	
	
/*	
	var cboCtaCte = new controles.combobox.ComboBoxAuto("services/", "viaticos.Viaticos", "autocompletarCtaCte");
	...
	*/
	...
	form.add(txtNroCheque, "Nro.cheque", null, "nro_cheque", null, {enabled: estado=="L", grupo: 1, item: {row: 14, column: 16, colSpan: 4}});
	...
	}, "fecha_cheque", null, {enabled: estado=="L", grupo: 1, item: {row: 15, column: 16, colSpan: 4}});
	

	

	//var formView = new qx.ui.form.renderer.Double(form);
	var formView1 = new controles.abstractrenderer.Grid(form, 50, 50, 1);
	...
	var formView2 = new controles.abstractrenderer.Grid(form, 50, 50, 2);
	...
	var formView3 = new controles.abstractrenderer.Grid(form, 50, 50, 3);
	...
	var formView4 = new controles.abstractrenderer.Grid(form, 50, 50, 4);
	...
	var cboLocalidadSel = new controles.combobox.ComboBoxAuto("services/", "viaticos.Viaticos", "autocompletarLocalidad");
	...
	this.add(groupbox, {right: 20, top: 0});
	

	
	/*
	var formView = new controles.abstractrenderer.Grid(form, 30, 30, 2);
	...
	*/
					...
					for (var x = 0; x < resultado.length; x++) {
						if (resultado[x].descrip == "documentacion_id") {
							...
							*/
							...
							strTopeMensual = "Con el importe de viático $" + numberformatMonto.format(model.subtotal_viatico2) + " se supera tope mensual. Permitir grabar viático?";
						}
					}
		...
		if (validationManager.getValid()) {
			var functionGrabar = function(e) {
				if (e) {
					...
					json.cant_dias = aux;
					model.json = qx.util.Json.stringify(json);
	...
	var lbldiario_12_dp = new qx.ui.basic.Label("");
	var lbldiario_34_dp = new qx.ui.basic.Label("");
	var lbldiario_dp = new qx.ui.basic.Label("");

	var lbldiario_12_fp = new qx.ui.basic.Label("");
	var lbldiario_34_fp = new qx.ui.basic.Label("");
	var lbldiario_fp = new qx.ui.basic.Label("");
	
	var lblSubtotal_viatico2 = new qx.ui.basic.Label("");
	lblSubtotal_viatico2.setTextAlign("right");
	lblSubtotal_viatico2.setWidth(120);
	
	gbDentro.add(lbldiario_12_dp, {left: 85, top: 10});
	gbDentro.add(lbldiario_34_dp, {left: 85, top: 40});
	gbDentro.add(lbldiario_dp, {left: 85, top: 70});

	gbFuera.add(lbldiario_12_fp, {left: 85, top: 10});
	gbFuera.add(lbldiario_34_fp, {left: 85, top: 40});
	gbFuera.add(lbldiario_fp, {left: 85, top: 70});
	...
	*/
	...
	txtDp34.setTabIndex(14);
	txtDp1.setTabIndex(15);
	txtFp12.setTabIndex(16);
	txtFp34.setTabIndex(17);
	txtFp1.setTabIndex(18);
	txtSubtotal_alojam2.setTabIndex(19);
	txtPasajes.setTabIndex(20);
	slbCtaCte.setTabIndex(21);
	txtNroCheque.setTabIndex(22);
	dtfFechacheque.setTabIndex(23);
