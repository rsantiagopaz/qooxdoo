qx.Class.define("viaticos.viaticos.windowViatico",
{
	...
	var calcularDias = function() {
		if (dtfFechadesde.getValue()!=null && txtHoradesde.getValue()!="" && dtfFechahasta.getValue()!=null && txtHorahasta.getValue()!="" && dtfFechahasta.getValue() - dtfFechadesde.getValue() >= 0) {
			...
			if (chkFueraprovincia.getValue()) {
				...
				fechahasta.setHours(parseInt(txtHorahasta.getValue().substr(0, 2)));
				fechahasta.setMinutes(parseInt(txtHorahasta.getValue().substr(3, 2)));
				...
				*/
				...
				fechadesde.setHours(parseInt(txtHoradesde.getValue().substr(0, 2)));
				fechadesde.setMinutes(parseInt(txtHoradesde.getValue().substr(3, 2)));
			...
			if (!lstPersonal.isSelectionEmpty() && lstPersonal.getSelection()[0].getUserData("datos").funcionario) {
				...
				if (chkConfuncionario.getValue()) {
					...
					txtMontoDiarioAlojam.setValue((chkFueraprovincia.getValue()) ? json.paramet.alojam_emp_fp : json.paramet.alojam_emp_dp);
				}
			}
	...
	*/
	...
	var cboVehiculo = new controles.combobox.ComboBoxAuto("services/", "viaticos.Viaticos", "autocompletarVehiculo");
	...
	chkConfuncionario.addListener("changeValue", function(e) {
		if (e.getData()&& estado!="L") {
			...
			txtCodigo_002fun.setEnabled(true);
	...
	form.add(chkConfuncionario, "", null, "con_funcionario", null, {enabled: estado!="L", grupo: 1, item: {row: 9, column: 16, colSpan: 4}});
	
	
	var cboFuncionario = new controles.combobox.ComboBoxAuto("services/", "viaticos.Viaticos", "autocompletarFuncionario");
	...
	}, "cboFuncionario", null, {enabled: false, grupo: 1, item: {row: 10, column: 16, colSpan: 13}});
	...
	form.add(txtCodigo_002fun, "Cód.002", function(value) {
		if (chkConfuncionario.getValue() && value <= 0) throw new qx.core.ValidationError("Validation Error", "Debe ingresar Código 002 del funcionario");
	}, "codigo_002fun", null, {enabled: false, grupo: 1, item: {row: 11, column: 16, colSpan: 3}});
	...
	options = {enabled: estado=="L", grupo: 1, item: {row: 13, column: 16, colSpan: 10}};
...
/*	
	...
	*/
	...
	options = {enabled: estado=="L", grupo: 1, item: {row: 14, column: 16, colSpan: 4}};
	...
	if (tipo_viatico == "A") {
		...
		var formView = new controles.abstractrenderer.Grid(form, 50, 50, 1);
	} else {
		this.setCaption(" - Reintegro");
		var formView = new controles.abstractrenderer.Grid(form, 50, 50);
	}
	...
	this.add(lblDiarioViatico, {left: 170, top: 279});
	...
	this.add(lblAdicional, {left: 290, top: 279});
	var lblAdicionallbl = new qx.ui.basic.Label("Adic.:");
	this.add(lblAdicionallbl, {left: 240, top: 279});
	...
	this.add(lblSubtotalViatico, {left: 350, top: 279});
	...
	this.add(lblDiarioAlojam, {left: 170, top: 311});
	...
	this.add(lblSubtotalAlojam, {left: 350, top: 311});
	...
	*/
	...
	if (id_viatico=="0") {
		...
		var ano, mes, dia;
		ano = parseInt(resultado.viatico.fecha_desde1.substr(0, 4));
		mes = parseInt(resultado.viatico.fecha_desde1.substr(5, 2)) - 1;
		dia = parseInt(resultado.viatico.fecha_desde1.substr(8, 2));
		...
		ano = parseInt(resultado.viatico.fecha_hasta1.substr(0, 4));
		mes = parseInt(resultado.viatico.fecha_hasta1.substr(5, 2)) - 1;
		dia = parseInt(resultado.viatico.fecha_hasta1.substr(8, 2));
