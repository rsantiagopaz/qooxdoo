...
*/
	...
	var rowDataActual;
	var boolImprimirViatico = false;
	var boolImprimirRendicion = false;
	...
	var contexto = this;
		...
		btn2.addListener("execute", function(e) {
			var window = new viaticos.viaticos.windowViatico(application, "0", "R", "E");
		...
		btn3.addListener("execute", function(e) {
			var window = new viaticos.viaticos.windowViatMen(application, "0", "M", "E");
			window.addListener("aceptado", function(e){
				boolImprimirViatico = true;
				actualizarViaticos(e.getData());
			});
		...
		btnRendir.addListener("execute", function(e){
			var window = new viaticos.viaticos.windowRendicion(application, rowDataActual.id_viatico);
			window.addListener("aceptado", function(e){
				boolImprimirRendicion = true;
				actualizarViaticos(e.getData());
			});
			window.setModal(true);
			application.getRoot().add(window);
		...
		btnCerrar.addListener("execute", function(e){
			dialog.Dialog.confirm("ATENCIÓN: Desea cerrar el viático seleccionado?", function(e){
				if (e) {
					...
					json.operaciones.push({fecha: aux, organismo_area: application.rowOrganismo_area.label, usuario: application._SYSusuario, operacion: "Cerrar"});
		...
		btnAnular.addListener("execute", function(e){
			dialog.Dialog.confirm("ATENCIÓN: Desea anular el viático seleccionado?", function(e){
				if (e) {
					...
					var aux = new Date();
					aux = aux.getFullYear() + "-" + (aux.getMonth() + 1) + "-" + aux.getDate();
					json.operaciones.push({fecha: aux, organismo_area: application.rowOrganismo_area.label, usuario: application._SYSusuario, operacion: "Anular"});
					json = qx.lang.Json.stringify(json);
